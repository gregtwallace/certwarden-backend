package storage_test

import (
	"certwarden-backend/pkg/acme"
	"certwarden-backend/pkg/domain/orders"
	"time"
)

var (
	ord99 = orders.Order{
		ID:             99,
		Certificate:    cert18,
		Location:       "https://acme-v02.api.letsencrypt.org/acme/order/red-2/red-99",
		Status:         "pending",
		KnownRevoked:   false,
		Error:          nil,
		Expires:        nil,
		DnsIdentifiers: []string{"desk.dude.example.com"},
		Authorizations: []string{"https://acme-v02.api.letsencrypt.org/acme/authz-v3/r-4199/"},
		Finalize:       "https://somefinalize.example.com/1234",
		FinalizedKey:   nil,
		CertificateUrl: nil,
		Pem:            nil,
		ValidFrom:      nil,
		ValidTo:        nil,
		CreatedAt:      time.Unix(1, 0),
		UpdatedAt:      time.Unix(2, 0),
		ChainRootCN:    new("xxxy"),
		Profile:        new("y1"),
		RenewalInfo:    nil,
	}

	// valid finalized key but that key has a null last_access
	ord150 = orders.Order{
		ID:             150,
		Certificate:    cert26,
		Location:       "https://acme-staging-v02.api.letsencrypt.org/acme/order/red-1/red-150",
		Status:         "valid",
		KnownRevoked:   false,
		Error:          nil,
		Expires:        new(time.Unix(1743775603, 0)),
		DnsIdentifiers: []string{"test008.test.example.com", "test009.test.example.com"},
		Authorizations: []string{"https://acme-staging-v02.api.letsencrypt.org/acme/authz/red-1/r-1274/", "https://acme-staging-v02.api.letsencrypt.org/acme/authz/red-1/r-3704/"},
		Finalize:       "",
		FinalizedKey:   &key55,
		CertificateUrl: new("https://acme-staging-v02.api.letsencrypt.org/acme/cert/red-150"),
		Pem: new(`-----BEGIN CERTIFICATE-----
red-150
-----END CERTIFICATE-----

-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----
`),
		ValidFrom:   new(time.Unix(1743167317, 0)),
		ValidTo:     new(time.Unix(1750943316, 0)),
		CreatedAt:   time.Unix(1743170803, 0),
		UpdatedAt:   time.Unix(1743170834, 0),
		ChainRootCN: new("(STAGING) Pretend Pear X1"),
		Profile:     nil,
		RenewalInfo: nil,
	}

	ord175 = orders.Order{
		ID:             175,
		Certificate:    cert28,
		Location:       "https://acme-staging-v02.api.letsencrypt.org/acme/order/red-1/red-175",
		Status:         "invalid",
		KnownRevoked:   false,
		Error:          nil,
		Expires:        new(time.Unix(1747188199, 0)),
		DnsIdentifiers: []string{"*.q1.alias.test.example.com", "*.q2.alias.test.example.com", "*.q3.alias.test.example.com", "*.q4.alias.test.example.com", "*.q5.alias.test.example.com", "*.q6.alias.test.example.com", "a1.alias.test.example.com", "q1.alias.test.example.com", "q2.alias.test.example.com", "q3.alias.test.example.com", "q4.alias.test.example.com", "q5.alias.test.example.com", "q6.alias.test.example.com"},
		Authorizations: []string{"https://acme-staging-v02.api.letsencrypt.org/acme/authz/red-1/r-6494", "https://acme-staging-v02.api.letsencrypt.org/acme/authz/red-1/r-6504", "https://acme-staging-v02.api.letsencrypt.org/acme/authz/red-1/r-6514", "https://acme-staging-v02.api.letsencrypt.org/acme/authz/red-1/r-6534", "https://acme-staging-v02.api.letsencrypt.org/acme/authz/red-1/r-6544", "https://acme-staging-v02.api.letsencrypt.org/acme/authz/red-1/r-6554", "https://acme-staging-v02.api.letsencrypt.org/acme/authz/red-1/r-6564", "https://acme-staging-v02.api.letsencrypt.org/acme/authz/red-1/r-6574", "https://acme-staging-v02.api.letsencrypt.org/acme/authz/red-1/r-6584", "https://acme-staging-v02.api.letsencrypt.org/acme/authz/red-1/r-6594", "https://acme-staging-v02.api.letsencrypt.org/acme/authz/red-1/r-6604", "https://acme-staging-v02.api.letsencrypt.org/acme/authz/red-1/r-6614", "https://acme-staging-v02.api.letsencrypt.org/acme/authz/red-1/r-6624"},
		Finalize:       "",
		FinalizedKey:   nil,
		CertificateUrl: nil,
		Pem:            nil,
		ValidFrom:      nil,
		ValidTo:        nil,
		CreatedAt:      time.Unix(1746583400, 0),
		UpdatedAt:      time.Unix(1746728924, 0),
		ChainRootCN:    nil,
		Profile:        nil,
		RenewalInfo:    nil,
	}

	ord186 = orders.Order{
		ID:             186,
		Certificate:    cert33,
		Location:       "https://acme-staging-v02.api.letsencrypt.org/acme/order/red-1/red-186",
		Status:         "invalid",
		KnownRevoked:   false,
		Error:          nil,
		Expires:        new(time.Unix(1776366393, 0)),
		DnsIdentifiers: []string{"test007.test.example2.com"},
		Authorizations: []string{"https://acme-staging-v02.api.letsencrypt.org/acme/authz/red-1/r-6993"},
		Finalize:       "https://acme-staging-v02.api.letsencrypt.org/acme/finalize/red-1/red-186",
		FinalizedKey:   nil,
		CertificateUrl: nil,
		Pem:            nil,
		ValidFrom:      nil,
		ValidTo:        nil,
		CreatedAt:      time.Unix(1775761593, 0),
		UpdatedAt:      time.Unix(1775761601, 0),
		ChainRootCN:    nil,
		Profile:        nil,
		RenewalInfo:    nil,
	}

	ord198 = orders.Order{
		ID:             198,
		Certificate:    cert33,
		Location:       "https://acme-staging-v02.api.letsencrypt.org/acme/order/red-1/red-198",
		Status:         "valid",
		KnownRevoked:   false,
		Error:          nil,
		Expires:        new(time.Unix(1779389985, 0)),
		DnsIdentifiers: []string{"test007.test.example2.com"},
		Authorizations: []string{"https://acme-staging-v02.api.letsencrypt.org/acme/authz/red-1/r-4643"},
		Finalize:       "https://acme-staging-v02.api.letsencrypt.org/acme/finalize/red-1/red-198",
		FinalizedKey:   &key69,
		CertificateUrl: new("https://acme-staging-v02.api.letsencrypt.org/acme/cert/red-198"),
		Pem: new(`-----BEGIN CERTIFICATE-----
red-198
-----END CERTIFICATE-----

-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----

-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----

-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----
`),
		ValidFrom:   new(time.Unix(1779382883, 0)),
		ValidTo:     new(time.Unix(1783270882, 0)),
		ChainRootCN: new("(STAGING) Pretend Pear X1"),
		CreatedAt:   time.Unix(1789386387, 0),
		UpdatedAt:   time.Unix(1779802225, 0),
		Profile:     new("tlsserver"),
		RenewalInfo: &orders.RenewalInfo{
			SuggestedWindow: struct {
				Start time.Time "json:\"start\""
				End   time.Time "json:\"end\""
			}{
				Start: time.Unix(1781937207, 0),
				End:   time.Unix(1782014897, 0),
			},
			ExplanationURL: nil,
			RetryAfter:     new(time.Unix(1779820852, 0)), // note: this was manually converted to zulu in the test db to ensure compatibility
		},
	}

	ord203 = orders.Order{
		ID:           203,
		Certificate:  cert18,
		Location:     "https://acme-v02.api.letsencrypt.org/acme/order/red-2/red-203",
		Status:       "valid",
		KnownRevoked: false,
		Error:        nil,
		Expires:      new(time.Unix(1779411544, 0)),
		DnsIdentifiers: []string{
			"desk.dude.example.com",
		},
		Authorizations: []string{
			"https://acme-v02.api.letsencrypt.org/acme/authz/red-2/r-6876",
		},
		Finalize:       "https://acme-v02.api.letsencrypt.org/acme/finalize/red-2/red-203",
		FinalizedKey:   &key31,
		CertificateUrl: new("https://acme-v02.api.letsencrypt.org/acme/cert/red-203"),
		Pem: new(`-----BEGIN CERTIFICATE-----
red-203
-----END CERTIFICATE-----

-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----

-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----

-----BEGIN CERTIFICATE-----

TT0mQ/r5XyA4MEAiabn7XJjvCERlF2dcn2wqJw+CreTkkQ2R
-----END CERTIFICATE-----
`),
		ValidFrom:   new(time.Unix(1779382921, 0)),
		ValidTo:     new(time.Unix(1783270920, 0)),
		CreatedAt:   time.Unix(1779386425, 0),
		UpdatedAt:   time.Unix(1779386440, 0),
		ChainRootCN: new("ISRG Root X1"),
		Profile:     new("tlsserver"),
		RenewalInfo: &orders.RenewalInfo{
			SuggestedWindow: struct {
				Start time.Time "json:\"start\""
				End   time.Time "json:\"end\""
			}{
				Start: time.Unix(1781937245, 0),
				End:   time.Unix(1782014934, 0),
			},
			ExplanationURL: nil,
			RetryAfter:     new(time.Unix(1779410250, 0)), //
		},
	}

	ord204 = orders.Order{
		ID:           204,
		Certificate:  cert35,
		Location:     "https://acme-staging-v02.api.letsencrypt.org/acme/order/red-1/red-204",
		Status:       "invalid",
		KnownRevoked: false,
		Error: new(acme.Error{
			Status: 522,
			Type:   "urn:ietf:params:acme:error:badNonce",
			Detail: "some kind of error info",
		}),
		Expires:        nil,
		DnsIdentifiers: []string{"test005.test.example2.com"},
		Authorizations: []string{"https://acme-staging-v02.api.letsencrypt.org/acme/authz/red-1/r-9773"},
		Finalize:       "https://acme-staging-v02.api.letsencrypt.org/acme/finalize/red-1/red-204",
		FinalizedKey:   nil,
		CertificateUrl: nil,
		Pem:            nil,
		ValidFrom:      nil,
		ValidTo:        nil,
		CreatedAt:      time.Unix(1779386773, 0),
		UpdatedAt:      time.Unix(1779386781, 0),
		ChainRootCN:    nil,
		Profile:        nil,
		RenewalInfo:    nil,
	}

	ord206 = orders.Order{
		ID:             206,
		Certificate:    cert33,
		Location:       "https://acme-staging-v02.api.letsencrypt.org/acme/order/red-1/red-206",
		Status:         "valid",
		KnownRevoked:   true,
		Error:          nil,
		Expires:        new(time.Unix(1779389990, 0)),
		DnsIdentifiers: []string{"test007.test.example2.com"},
		Authorizations: []string{"https://acme-staging-v02.api.letsencrypt.org/acme/authz/red-1/r-4643"},
		Finalize:       "https://acme-staging-v02.api.letsencrypt.org/acme/finalize/red-1/red-206",
		FinalizedKey:   &key69,
		CertificateUrl: new("https://acme-staging-v02.api.letsencrypt.org/acme/cert/red-206"),
		Pem: new(`-----BEGIN CERTIFICATE-----
red-206
-----END CERTIFICATE-----

-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----

-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----

-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----
`),
		ValidFrom:   new(time.Unix(1779382890, 0)),
		ValidTo:     new(time.Unix(1783270890, 0)),
		CreatedAt:   time.Unix(1779386390, 0),
		UpdatedAt:   time.Unix(1779802230, 0),
		ChainRootCN: new("(STAGING) Pretend Pear X1"),
		Profile:     new("tlsserver"),
		RenewalInfo: &orders.RenewalInfo{
			SuggestedWindow: struct {
				Start time.Time "json:\"start\""
				End   time.Time "json:\"end\""
			}{
				Start: time.Unix(1781937207, 0),
				End:   time.Unix(1782014897, 0),
			},
			ExplanationURL: new("https://example.com/explain"),
			RetryAfter:     new(time.Unix(1779820852, 0)), //
		},
	}
)
