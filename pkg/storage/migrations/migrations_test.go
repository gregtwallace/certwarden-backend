...
func TestSetupGoose(t *testing.T) {
	t.Run("empty file", func(t *testing.T) {
		thisTestFolder := tempFileStorage + "migrate_goose_empty"
		...
		if err != nil {
			t.Fatalf("failed to setup goose: %s", err)
		}

		// test: verify table exists and current version is 0
		q := `
	  ...
	  `
		...
		maxVer := -1
		err = rows.Scan(&maxVer)
		if err != nil {
			t.Fatalf("failed to scan max goose_db_version is_applied version_id (%s)", err)
		}
		if maxVer != 0 {
			t.Errorf("goose max ver expected '%d' but got '%d'", 0, maxVer)
		}
	})

	t.Run("testdata_v11", func(t *testing.T) {
		thisTestFolder := tempFileStorage + "migrate_goose_v11"
	  ...
	  `
		...
		if maxVer != 11 {
			t.Errorf("goose max ver expected '%d' but got '%d'", 11, maxVer)
		}
	})

	// TODO: confirm no-op on existing goose_db_version table
}
