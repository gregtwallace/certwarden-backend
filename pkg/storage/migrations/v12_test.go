...
func TestDBVersion12(t *testing.T) {
	...
	err = goose.UpToContext(ctx, db, "sql", 8)
	if err != nil {
		t.Fatalf("goose failed to up migrate v7 -> v8: %s", err)
	}

	insertDataV8(t, db)
	validateDataV8(t, db)

	err = goose.UpToContext(ctx, db, "sql", 9)
	if err != nil {
		t.Fatalf("goose failed to up migrate v8 -> v9: %s", err)
	}

	insertDataV9(t, db)
	validateDataV9(t, db)

	err = goose.UpToContext(ctx, db, "sql", 10)
	if err != nil {
		t.Fatalf("goose failed to up migrate v9 -> v10: %s", err)
	}

	insertDataV10(t, db)
	validateDataV10(t, db)

	err = goose.UpToContext(ctx, db, "sql", 11)
	if err != nil {
		t.Fatalf("goose failed to up migrate v10 -> v11: %s", err)
	}

	insertDataV11(t, db)
	validateDataV11(t, db)

	err = goose.UpToContext(ctx, db, "sql", 12)
	if err != nil {
		t.Fatalf("goose failed to up migrate v11 -> v12: %s", err)
	}

	insertDataV12(t, db)
	validateDataV12(t, db)

	//
	// REVERSE AND GO DOWN
	//
	err = goose.DownToContext(ctx, db, "sql", 11)
	if err != nil {
		t.Fatalf("goose failed to down migrate v12 -> v11: %s", err)
	}

	validateDataV11(t, db)

	err = goose.DownToContext(ctx, db, "sql", 10)
	if err != nil {
		t.Fatalf("goose failed to down migrate v11 -> v10: %s", err)
	}

	validateDataV10(t, db)

	err = goose.DownToContext(ctx, db, "sql", 9)
	if err != nil {
		t.Fatalf("goose failed to down migrate v10 -> v9: %s", err)
	}

	validateDataV9(t, db)

	err = goose.DownToContext(ctx, db, "sql", 8)
	if err != nil {
		t.Fatalf("goose failed to down migrate v9 -> v8: %s", err)
	}

	validateDataV8(t, db)

	err = goose.DownToContext(ctx, db, "sql", 7)
	if err != nil {
		t.Fatalf("goose failed to down migrate v8 -> v7: %s", err)
	}
