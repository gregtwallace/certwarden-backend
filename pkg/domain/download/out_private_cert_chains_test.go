package download

import (
	"certwarden-backend/pkg/output"
	"testing"
)

func TestOutPrivateCertChainViaHeader(t *testing.T) {
	// create testing service
	app := makeFakeApp(t)
	service, err := NewService(app)
	if err != nil {
		t.Fatal(err)
	}

	// Test: No header provided
	oneTest(t, service.DownloadPrivateCertChainViaHeader, nil, nil, "test-a", "", output.JsonErrUnauthorized)
	oneTest(t, service.DownloadPrivateCertChainViaHeader, nil, nil, "test-b", "", output.JsonErrUnauthorized)
	oneTest(t, service.DownloadPrivateCertChainViaHeader, nil, nil, "test-c", "", output.JsonErrUnauthorized)
	oneTest(t, service.DownloadPrivateCertChainViaHeader, nil, nil, "test-d", "", output.JsonErrUnauthorized)
	oneTest(t, service.DownloadPrivateCertChainViaHeader, nil, nil, "test-e", "", output.JsonErrUnauthorized)
	oneTest(t, service.DownloadPrivateCertChainViaHeader, nil, nil, "test-g", "", output.JsonErrUnauthorized)

	// Test: blank/empty apikey provided
	apiKey := ""
	oneTest(t, service.DownloadPrivateCertChainViaHeader, &apiKey, nil, "test-a", "", output.JsonErrUnauthorized)
	oneTest(t, service.DownloadPrivateCertChainViaHeader, &apiKey, nil, "test-b", "", output.JsonErrUnauthorized)
	oneTest(t, service.DownloadPrivateCertChainViaHeader, &apiKey, nil, "test-c", "", output.JsonErrUnauthorized)
	oneTest(t, service.DownloadPrivateCertChainViaHeader, &apiKey, nil, "test-d", "", output.JsonErrUnauthorized)
	oneTest(t, service.DownloadPrivateCertChainViaHeader, &apiKey, nil, "test-e", "", output.JsonErrUnauthorized)
	oneTest(t, service.DownloadPrivateCertChainViaHeader, &apiKey, nil, "test-g", "", output.JsonErrUnauthorized)

	// Test: incorrect apikey provided
	apiKey = "something"
	oneTest(t, service.DownloadPrivateCertChainViaHeader, &apiKey, nil, "test-a", "", output.JsonErrUnauthorized)
	oneTest(t, service.DownloadPrivateCertChainViaHeader, &apiKey, nil, "test-b", "", output.JsonErrUnauthorized)
	oneTest(t, service.DownloadPrivateCertChainViaHeader, &apiKey, nil, "test-c", "", output.JsonErrUnauthorized)
	oneTest(t, service.DownloadPrivateCertChainViaHeader, &apiKey, nil, "test-d", "", output.JsonErrUnauthorized)
	oneTest(t, service.DownloadPrivateCertChainViaHeader, &apiKey, nil, "test-e", "", output.JsonErrUnauthorized)
	oneTest(t, service.DownloadPrivateCertChainViaHeader, &apiKey, nil, "test-g", "", output.JsonErrUnauthorized)
	apiKey = "something.something"
	oneTest(t, service.DownloadPrivateCertChainViaHeader, &apiKey, nil, "test-a", "", output.JsonErrUnauthorized)
	oneTest(t, service.DownloadPrivateCertChainViaHeader, &apiKey, nil, "test-b", "", output.JsonErrUnauthorized)
	oneTest(t, service.DownloadPrivateCertChainViaHeader, &apiKey, nil, "test-c", "", output.JsonErrUnauthorized)
	oneTest(t, service.DownloadPrivateCertChainViaHeader, &apiKey, nil, "test-d", "", output.JsonErrUnauthorized)
	oneTest(t, service.DownloadPrivateCertChainViaHeader, &apiKey, nil, "test-e", "", output.JsonErrUnauthorized)
	oneTest(t, service.DownloadPrivateCertChainViaHeader, &apiKey, nil, "test-g", "", output.JsonErrUnauthorized)

	// Test: just one of the apikeys
	apiKey = "c-abc"
	oneTest(t, service.DownloadPrivateCertChainViaHeader, &apiKey, nil, "test-a", "", output.JsonErrUnauthorized)
	oneTest(t, service.DownloadPrivateCertChainViaHeader, &apiKey, nil, "test-b", "", output.JsonErrUnauthorized)
	oneTest(t, service.DownloadPrivateCertChainViaHeader, &apiKey, nil, "test-c", "", output.JsonErrUnauthorized)
	oneTest(t, service.DownloadPrivateCertChainViaHeader, &apiKey, nil, "test-d", "", output.JsonErrUnauthorized)
	oneTest(t, service.DownloadPrivateCertChainViaHeader, &apiKey, nil, "test-e", "", output.JsonErrUnauthorized)
	oneTest(t, service.DownloadPrivateCertChainViaHeader, &apiKey, nil, "test-g", "", output.JsonErrUnauthorized)
	apiKey = "k-123"
	oneTest(t, service.DownloadPrivateCertChainViaHeader, &apiKey, nil, "test-a", "", output.JsonErrUnauthorized)
	oneTest(t, service.DownloadPrivateCertChainViaHeader, &apiKey, nil, "test-b", "", output.JsonErrUnauthorized)
	oneTest(t, service.DownloadPrivateCertChainViaHeader, &apiKey, nil, "test-c", "", output.JsonErrUnauthorized)
	oneTest(t, service.DownloadPrivateCertChainViaHeader, &apiKey, nil, "test-d", "", output.JsonErrUnauthorized)
	oneTest(t, service.DownloadPrivateCertChainViaHeader, &apiKey, nil, "test-e", "", output.JsonErrUnauthorized)
	oneTest(t, service.DownloadPrivateCertChainViaHeader, &apiKey, nil, "test-g", "", output.JsonErrUnauthorized)

	// Test: apikey variants
	apiKey = ".k-123"
	oneTest(t, service.DownloadPrivateCertChainViaHeader, &apiKey, nil, "test-a", "", output.JsonErrUnauthorized)
	oneTest(t, service.DownloadPrivateCertChainViaHeader, &apiKey, nil, "test-b", "", output.JsonErrUnauthorized)
	oneTest(t, service.DownloadPrivateCertChainViaHeader, &apiKey, nil, "test-c", "", output.JsonErrUnauthorized)
	oneTest(t, service.DownloadPrivateCertChainViaHeader, &apiKey, nil, "test-d", "", output.JsonErrUnauthorized)
	oneTest(t, service.DownloadPrivateCertChainViaHeader, &apiKey, nil, "test-e", "", output.JsonErrUnauthorized)
	oneTest(t, service.DownloadPrivateCertChainViaHeader, &apiKey, nil, "test-g", "", output.JsonErrUnauthorized)
	apiKey = ".c-abc"
	oneTest(t, service.DownloadPrivateCertChainViaHeader, &apiKey, nil, "test-a", "", output.JsonErrUnauthorized)
	oneTest(t, service.DownloadPrivateCertChainViaHeader, &apiKey, nil, "test-b", "", output.JsonErrUnauthorized)
	oneTest(t, service.DownloadPrivateCertChainViaHeader, &apiKey, nil, "test-c", "", output.JsonErrUnauthorized)
	oneTest(t, service.DownloadPrivateCertChainViaHeader, &apiKey, nil, "test-d", "", output.JsonErrUnauthorized)
	oneTest(t, service.DownloadPrivateCertChainViaHeader, &apiKey, nil, "test-e", "", output.JsonErrUnauthorized)
	oneTest(t, service.DownloadPrivateCertChainViaHeader, &apiKey, nil, "test-g", "", output.JsonErrUnauthorized)
	apiKey = "k-123."
	oneTest(t, service.DownloadPrivateCertChainViaHeader, &apiKey, nil, "test-a", "", output.JsonErrUnauthorized)
	oneTest(t, service.DownloadPrivateCertChainViaHeader, &apiKey, nil, "test-b", "", output.JsonErrUnauthorized)
	oneTest(t, service.DownloadPrivateCertChainViaHeader, &apiKey, nil, "test-c", "", output.JsonErrUnauthorized)
	oneTest(t, service.DownloadPrivateCertChainViaHeader, &apiKey, nil, "test-d", "", output.JsonErrUnauthorized)
	oneTest(t, service.DownloadPrivateCertChainViaHeader, &apiKey, nil, "test-e", "", output.JsonErrUnauthorized)
	oneTest(t, service.DownloadPrivateCertChainViaHeader, &apiKey, nil, "test-g", "", output.JsonErrUnauthorized)
	apiKey = "c-abc."
	oneTest(t, service.DownloadPrivateCertChainViaHeader, &apiKey, nil, "test-a", "", output.JsonErrUnauthorized)
	oneTest(t, service.DownloadPrivateCertChainViaHeader, &apiKey, nil, "test-b", "", output.JsonErrUnauthorized)
	oneTest(t, service.DownloadPrivateCertChainViaHeader, &apiKey, nil, "test-c", "", output.JsonErrUnauthorized)
	oneTest(t, service.DownloadPrivateCertChainViaHeader, &apiKey, nil, "test-d", "", output.JsonErrUnauthorized)
	oneTest(t, service.DownloadPrivateCertChainViaHeader, &apiKey, nil, "test-e", "", output.JsonErrUnauthorized)
	oneTest(t, service.DownloadPrivateCertChainViaHeader, &apiKey, nil, "test-g", "", output.JsonErrUnauthorized)
	apiKey = "123.k-123"
	oneTest(t, service.DownloadPrivateCertChainViaHeader, &apiKey, nil, "test-a", "", output.JsonErrUnauthorized)
	oneTest(t, service.DownloadPrivateCertChainViaHeader, &apiKey, nil, "test-b", "", output.JsonErrUnauthorized)
	oneTest(t, service.DownloadPrivateCertChainViaHeader, &apiKey, nil, "test-c", "", output.JsonErrUnauthorized)
	oneTest(t, service.DownloadPrivateCertChainViaHeader, &apiKey, nil, "test-d", "", output.JsonErrUnauthorized)
	oneTest(t, service.DownloadPrivateCertChainViaHeader, &apiKey, nil, "test-e", "", output.JsonErrUnauthorized)
	oneTest(t, service.DownloadPrivateCertChainViaHeader, &apiKey, nil, "test-g", "", output.JsonErrUnauthorized)
	apiKey = "k-123.123"
	oneTest(t, service.DownloadPrivateCertChainViaHeader, &apiKey, nil, "test-a", "", output.JsonErrUnauthorized)
	oneTest(t, service.DownloadPrivateCertChainViaHeader, &apiKey, nil, "test-b", "", output.JsonErrUnauthorized)
	oneTest(t, service.DownloadPrivateCertChainViaHeader, &apiKey, nil, "test-c", "", output.JsonErrUnauthorized)
	oneTest(t, service.DownloadPrivateCertChainViaHeader, &apiKey, nil, "test-d", "", output.JsonErrUnauthorized)
	oneTest(t, service.DownloadPrivateCertChainViaHeader, &apiKey, nil, "test-e", "", output.JsonErrUnauthorized)
	oneTest(t, service.DownloadPrivateCertChainViaHeader, &apiKey, nil, "test-g", "", output.JsonErrUnauthorized)
	apiKey = "123.c-abc"
	oneTest(t, service.DownloadPrivateCertChainViaHeader, &apiKey, nil, "test-a", "", output.JsonErrUnauthorized)
	oneTest(t, service.DownloadPrivateCertChainViaHeader, &apiKey, nil, "test-b", "", output.JsonErrUnauthorized)
	oneTest(t, service.DownloadPrivateCertChainViaHeader, &apiKey, nil, "test-c", "", output.JsonErrUnauthorized)
	oneTest(t, service.DownloadPrivateCertChainViaHeader, &apiKey, nil, "test-d", "", output.JsonErrUnauthorized)
	oneTest(t, service.DownloadPrivateCertChainViaHeader, &apiKey, nil, "test-e", "", output.JsonErrUnauthorized)
	oneTest(t, service.DownloadPrivateCertChainViaHeader, &apiKey, nil, "test-g", "", output.JsonErrUnauthorized)
	apiKey = "c-abc.123"
	oneTest(t, service.DownloadPrivateCertChainViaHeader, &apiKey, nil, "test-a", "", output.JsonErrUnauthorized)
	oneTest(t, service.DownloadPrivateCertChainViaHeader, &apiKey, nil, "test-b", "", output.JsonErrUnauthorized)
	oneTest(t, service.DownloadPrivateCertChainViaHeader, &apiKey, nil, "test-c", "", output.JsonErrUnauthorized)
	oneTest(t, service.DownloadPrivateCertChainViaHeader, &apiKey, nil, "test-d", "", output.JsonErrUnauthorized)
	oneTest(t, service.DownloadPrivateCertChainViaHeader, &apiKey, nil, "test-e", "", output.JsonErrUnauthorized)
	oneTest(t, service.DownloadPrivateCertChainViaHeader, &apiKey, nil, "test-g", "", output.JsonErrUnauthorized)

	// Test: correct apikey provided but via url
	apiKey = "c-abc.k-123"
	oneTest(t, service.DownloadPrivateCertChainViaHeader, nil, &apiKey, "test-a", "", output.JsonErrUnauthorized)
	// `b` doesn't have a non-new apikey
	oneTest(t, service.DownloadPrivateCertChainViaHeader, nil, &apiKey, "test-c", "", output.JsonErrUnauthorized)
	// `d` doesnt have a any correct apikey
	oneTest(t, service.DownloadPrivateCertChainViaHeader, nil, &apiKey, "test-e", "", output.JsonErrUnauthorized)
	oneTest(t, service.DownloadPrivateCertChainViaHeader, nil, &apiKey, "test-g", "", output.JsonErrUnauthorized)

	// Test: correct new apikey provided but via url
	apiKey = "c-abc-new.k-123-new"
	oneTest(t, service.DownloadPrivateCertChainViaHeader, nil, &apiKey, "test-b", "", output.JsonErrUnauthorized)
	oneTest(t, service.DownloadPrivateCertChainViaHeader, nil, &apiKey, "test-e", "", output.JsonErrUnauthorized)

	// Test: correct apikey provided
	apiKey = "c-abc.k-123"
	oneTest(t, service.DownloadPrivateCertChainViaHeader, &apiKey, nil, "test-a", `-----BEGIN RSA PRIVATE KEY-----

-----END RSA PRIVATE KEY-----
-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----
-----BEGIN CERTIFICATE-----

sp5CbPfXs9p7/ovby8Fmd4QY+CVXwlYrZZnI
-----END CERTIFICATE-----
-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`, nil)
	// `b` doesn't have a non-new apikey
	oneTest(t, service.DownloadPrivateCertChainViaHeader, &apiKey, nil, "test-c", `-----BEGIN RSA PRIVATE KEY-----

-----END RSA PRIVATE KEY-----
-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----
-----BEGIN CERTIFICATE-----

roblaZUgYh2sf2XTG1U12LfTJ7LS9X3mC8HO
-----END CERTIFICATE-----
-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`, nil)
	// // `d` doesnt have a any correct apikey
	oneTest(t, service.DownloadPrivateCertChainViaHeader, &apiKey, nil, "test-e", `-----BEGIN RSA PRIVATE KEY-----

N/VC8wvORy4lKBkAeRy0oK+9o6R5mdzP3Fjpz9I4
-----END RSA PRIVATE KEY-----
-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----
-----BEGIN CERTIFICATE-----

g1/KbwHRr8PkIHTFtGbFNvZzjbrYb/FhHbSA
-----END CERTIFICATE-----
-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`, nil)
	oneTest(t, service.DownloadPrivateCertChainViaHeader, &apiKey, nil, "test-g", `-----BEGIN RSA PRIVATE KEY-----

j1f1P6e7Khe0uXD3N+r34piMQT0WX0po2rf16x0i
-----END RSA PRIVATE KEY-----
-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----
-----BEGIN CERTIFICATE-----

HZtRvh8TK5VI0J64ed//xMalyNSeh9gva5KB
-----END CERTIFICATE-----
-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`, nil)

	// Test: correct new apikey provided
	apiKey = "c-abc-new.k-123-new"
	oneTest(t, service.DownloadPrivateCertChainViaHeader, &apiKey, nil, "test-b", `-----BEGIN RSA PRIVATE KEY-----

5URzNI/wCwqQD6D98UNKRJhD4MrDQlBBA9PYqnab
-----END RSA PRIVATE KEY-----
-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----
-----BEGIN CERTIFICATE-----

lu3c8NbPDv+Dl+nlovC7VvHm6Uq8aKcPpCuh
-----END CERTIFICATE-----
-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`, nil)
	oneTest(t, service.DownloadPrivateCertChainViaHeader, &apiKey, nil, "test-e", `-----BEGIN RSA PRIVATE KEY-----

N/VC8wvORy4lKBkAeRy0oK+9o6R5mdzP3Fjpz9I4
-----END RSA PRIVATE KEY-----
-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----
-----BEGIN CERTIFICATE-----

g1/KbwHRr8PkIHTFtGbFNvZzjbrYb/FhHbSA
-----END CERTIFICATE-----
-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`, nil)
}

func TestOutPrivateCertChainViaURL(t *testing.T) {
	// create testing service
	app := makeFakeApp(t)
	service, err := NewService(app)
	if err != nil {
		t.Fatal(err)
	}

	// Test: No header provided
	oneTest(t, service.DownloadPrivateCertChainViaUrl, nil, nil, "test-a", "", output.JsonErrUnauthorized)
	oneTest(t, service.DownloadPrivateCertChainViaUrl, nil, nil, "test-b", "", output.JsonErrUnauthorized)
	oneTest(t, service.DownloadPrivateCertChainViaUrl, nil, nil, "test-c", "", output.JsonErrUnauthorized)
	oneTest(t, service.DownloadPrivateCertChainViaUrl, nil, nil, "test-d", "", output.JsonErrUnauthorized)
	oneTest(t, service.DownloadPrivateCertChainViaUrl, nil, nil, "test-e", "", output.JsonErrUnauthorized)
	oneTest(t, service.DownloadPrivateCertChainViaUrl, nil, nil, "test-g", "", output.JsonErrUnauthorized)

	// Test: blank/empty apikey provided
	apiKey := ""
	oneTest(t, service.DownloadPrivateCertChainViaUrl, nil, &apiKey, "test-a", "", output.JsonErrUnauthorized)
	oneTest(t, service.DownloadPrivateCertChainViaUrl, nil, &apiKey, "test-b", "", output.JsonErrUnauthorized)
	oneTest(t, service.DownloadPrivateCertChainViaUrl, nil, &apiKey, "test-c", "", output.JsonErrUnauthorized)
	oneTest(t, service.DownloadPrivateCertChainViaUrl, nil, &apiKey, "test-d", "", output.JsonErrUnauthorized)
	oneTest(t, service.DownloadPrivateCertChainViaUrl, nil, &apiKey, "test-e", "", output.JsonErrUnauthorized)
	oneTest(t, service.DownloadPrivateCertChainViaUrl, nil, &apiKey, "test-g", "", output.JsonErrUnauthorized)

	// Test: incorrect apikey provided
	apiKey = "something"
	oneTest(t, service.DownloadPrivateCertChainViaUrl, nil, &apiKey, "test-a", "", output.JsonErrUnauthorized)
	oneTest(t, service.DownloadPrivateCertChainViaUrl, nil, &apiKey, "test-b", "", output.JsonErrUnauthorized)
	oneTest(t, service.DownloadPrivateCertChainViaUrl, nil, &apiKey, "test-c", "", output.JsonErrUnauthorized)
	oneTest(t, service.DownloadPrivateCertChainViaUrl, nil, &apiKey, "test-d", "", output.JsonErrUnauthorized)
	oneTest(t, service.DownloadPrivateCertChainViaUrl, nil, &apiKey, "test-e", "", output.JsonErrUnauthorized)
	oneTest(t, service.DownloadPrivateCertChainViaUrl, nil, &apiKey, "test-g", "", output.JsonErrUnauthorized)
	apiKey = "something.something"
	oneTest(t, service.DownloadPrivateCertChainViaUrl, nil, &apiKey, "test-a", "", output.JsonErrUnauthorized)
	oneTest(t, service.DownloadPrivateCertChainViaUrl, nil, &apiKey, "test-b", "", output.JsonErrUnauthorized)
	oneTest(t, service.DownloadPrivateCertChainViaUrl, nil, &apiKey, "test-c", "", output.JsonErrUnauthorized)
	oneTest(t, service.DownloadPrivateCertChainViaUrl, nil, &apiKey, "test-d", "", output.JsonErrUnauthorized)
	oneTest(t, service.DownloadPrivateCertChainViaUrl, nil, &apiKey, "test-e", "", output.JsonErrUnauthorized)
	oneTest(t, service.DownloadPrivateCertChainViaUrl, nil, &apiKey, "test-g", "", output.JsonErrUnauthorized)

	// Test: just one of the apikeys
	apiKey = "c-abc"
	oneTest(t, service.DownloadPrivateCertChainViaUrl, nil, &apiKey, "test-a", "", output.JsonErrUnauthorized)
	oneTest(t, service.DownloadPrivateCertChainViaUrl, nil, &apiKey, "test-b", "", output.JsonErrUnauthorized)
	oneTest(t, service.DownloadPrivateCertChainViaUrl, nil, &apiKey, "test-c", "", output.JsonErrUnauthorized)
	oneTest(t, service.DownloadPrivateCertChainViaUrl, nil, &apiKey, "test-d", "", output.JsonErrUnauthorized)
	oneTest(t, service.DownloadPrivateCertChainViaUrl, nil, &apiKey, "test-e", "", output.JsonErrUnauthorized)
	oneTest(t, service.DownloadPrivateCertChainViaUrl, nil, &apiKey, "test-g", "", output.JsonErrUnauthorized)
	apiKey = "k-123"
	oneTest(t, service.DownloadPrivateCertChainViaUrl, nil, &apiKey, "test-a", "", output.JsonErrUnauthorized)
	oneTest(t, service.DownloadPrivateCertChainViaUrl, nil, &apiKey, "test-b", "", output.JsonErrUnauthorized)
	oneTest(t, service.DownloadPrivateCertChainViaUrl, nil, &apiKey, "test-c", "", output.JsonErrUnauthorized)
	oneTest(t, service.DownloadPrivateCertChainViaUrl, nil, &apiKey, "test-d", "", output.JsonErrUnauthorized)
	oneTest(t, service.DownloadPrivateCertChainViaUrl, nil, &apiKey, "test-e", "", output.JsonErrUnauthorized)
	oneTest(t, service.DownloadPrivateCertChainViaUrl, nil, &apiKey, "test-g", "", output.JsonErrUnauthorized)

	// Test: apikey variants
	apiKey = ".k-123"
	oneTest(t, service.DownloadPrivateCertChainViaUrl, nil, &apiKey, "test-a", "", output.JsonErrUnauthorized)
	oneTest(t, service.DownloadPrivateCertChainViaUrl, nil, &apiKey, "test-b", "", output.JsonErrUnauthorized)
	oneTest(t, service.DownloadPrivateCertChainViaUrl, nil, &apiKey, "test-c", "", output.JsonErrUnauthorized)
	oneTest(t, service.DownloadPrivateCertChainViaUrl, nil, &apiKey, "test-d", "", output.JsonErrUnauthorized)
	oneTest(t, service.DownloadPrivateCertChainViaUrl, nil, &apiKey, "test-e", "", output.JsonErrUnauthorized)
	oneTest(t, service.DownloadPrivateCertChainViaUrl, nil, &apiKey, "test-g", "", output.JsonErrUnauthorized)
	apiKey = ".c-abc"
	oneTest(t, service.DownloadPrivateCertChainViaUrl, nil, &apiKey, "test-a", "", output.JsonErrUnauthorized)
	oneTest(t, service.DownloadPrivateCertChainViaUrl, nil, &apiKey, "test-b", "", output.JsonErrUnauthorized)
	oneTest(t, service.DownloadPrivateCertChainViaUrl, nil, &apiKey, "test-c", "", output.JsonErrUnauthorized)
	oneTest(t, service.DownloadPrivateCertChainViaUrl, nil, &apiKey, "test-d", "", output.JsonErrUnauthorized)
	oneTest(t, service.DownloadPrivateCertChainViaUrl, nil, &apiKey, "test-e", "", output.JsonErrUnauthorized)
	oneTest(t, service.DownloadPrivateCertChainViaUrl, nil, &apiKey, "test-g", "", output.JsonErrUnauthorized)
	apiKey = "k-123."
	oneTest(t, service.DownloadPrivateCertChainViaUrl, nil, &apiKey, "test-a", "", output.JsonErrUnauthorized)
	oneTest(t, service.DownloadPrivateCertChainViaUrl, nil, &apiKey, "test-b", "", output.JsonErrUnauthorized)
	oneTest(t, service.DownloadPrivateCertChainViaUrl, nil, &apiKey, "test-c", "", output.JsonErrUnauthorized)
	oneTest(t, service.DownloadPrivateCertChainViaUrl, nil, &apiKey, "test-d", "", output.JsonErrUnauthorized)
	oneTest(t, service.DownloadPrivateCertChainViaUrl, nil, &apiKey, "test-e", "", output.JsonErrUnauthorized)
	oneTest(t, service.DownloadPrivateCertChainViaUrl, nil, &apiKey, "test-g", "", output.JsonErrUnauthorized)
	apiKey = "c-abc."
	oneTest(t, service.DownloadPrivateCertChainViaUrl, nil, &apiKey, "test-a", "", output.JsonErrUnauthorized)
	oneTest(t, service.DownloadPrivateCertChainViaUrl, nil, &apiKey, "test-b", "", output.JsonErrUnauthorized)
	oneTest(t, service.DownloadPrivateCertChainViaUrl, nil, &apiKey, "test-c", "", output.JsonErrUnauthorized)
	oneTest(t, service.DownloadPrivateCertChainViaUrl, nil, &apiKey, "test-d", "", output.JsonErrUnauthorized)
	oneTest(t, service.DownloadPrivateCertChainViaUrl, nil, &apiKey, "test-e", "", output.JsonErrUnauthorized)
	oneTest(t, service.DownloadPrivateCertChainViaUrl, nil, &apiKey, "test-g", "", output.JsonErrUnauthorized)
	apiKey = "123.k-123"
	oneTest(t, service.DownloadPrivateCertChainViaUrl, nil, &apiKey, "test-a", "", output.JsonErrUnauthorized)
	oneTest(t, service.DownloadPrivateCertChainViaUrl, nil, &apiKey, "test-b", "", output.JsonErrUnauthorized)
	oneTest(t, service.DownloadPrivateCertChainViaUrl, nil, &apiKey, "test-c", "", output.JsonErrUnauthorized)
	oneTest(t, service.DownloadPrivateCertChainViaUrl, nil, &apiKey, "test-d", "", output.JsonErrUnauthorized)
	oneTest(t, service.DownloadPrivateCertChainViaUrl, nil, &apiKey, "test-e", "", output.JsonErrUnauthorized)
	oneTest(t, service.DownloadPrivateCertChainViaUrl, nil, &apiKey, "test-g", "", output.JsonErrUnauthorized)
	apiKey = "k-123.123"
	oneTest(t, service.DownloadPrivateCertChainViaUrl, nil, &apiKey, "test-a", "", output.JsonErrUnauthorized)
	oneTest(t, service.DownloadPrivateCertChainViaUrl, nil, &apiKey, "test-b", "", output.JsonErrUnauthorized)
	oneTest(t, service.DownloadPrivateCertChainViaUrl, nil, &apiKey, "test-c", "", output.JsonErrUnauthorized)
	oneTest(t, service.DownloadPrivateCertChainViaUrl, nil, &apiKey, "test-d", "", output.JsonErrUnauthorized)
	oneTest(t, service.DownloadPrivateCertChainViaUrl, nil, &apiKey, "test-e", "", output.JsonErrUnauthorized)
	oneTest(t, service.DownloadPrivateCertChainViaUrl, nil, &apiKey, "test-g", "", output.JsonErrUnauthorized)
	apiKey = "123.c-abc"
	oneTest(t, service.DownloadPrivateCertChainViaUrl, nil, &apiKey, "test-a", "", output.JsonErrUnauthorized)
	oneTest(t, service.DownloadPrivateCertChainViaUrl, nil, &apiKey, "test-b", "", output.JsonErrUnauthorized)
	oneTest(t, service.DownloadPrivateCertChainViaUrl, nil, &apiKey, "test-c", "", output.JsonErrUnauthorized)
	oneTest(t, service.DownloadPrivateCertChainViaUrl, nil, &apiKey, "test-d", "", output.JsonErrUnauthorized)
	oneTest(t, service.DownloadPrivateCertChainViaUrl, nil, &apiKey, "test-e", "", output.JsonErrUnauthorized)
	oneTest(t, service.DownloadPrivateCertChainViaUrl, nil, &apiKey, "test-g", "", output.JsonErrUnauthorized)
	apiKey = "c-abc.123"
	oneTest(t, service.DownloadPrivateCertChainViaUrl, nil, &apiKey, "test-a", "", output.JsonErrUnauthorized)
	oneTest(t, service.DownloadPrivateCertChainViaUrl, nil, &apiKey, "test-b", "", output.JsonErrUnauthorized)
	oneTest(t, service.DownloadPrivateCertChainViaUrl, nil, &apiKey, "test-c", "", output.JsonErrUnauthorized)
	oneTest(t, service.DownloadPrivateCertChainViaUrl, nil, &apiKey, "test-d", "", output.JsonErrUnauthorized)
	oneTest(t, service.DownloadPrivateCertChainViaUrl, nil, &apiKey, "test-e", "", output.JsonErrUnauthorized)
	oneTest(t, service.DownloadPrivateCertChainViaUrl, nil, &apiKey, "test-g", "", output.JsonErrUnauthorized)

	// Test: correct apikey provided
	apiKey = "c-abc.k-123"
	oneTest(t, service.DownloadPrivateCertChainViaUrl, nil, &apiKey, "test-a", "", output.JsonErrUnauthorized)
	// `b` doesn't have a non-new apikey
	oneTest(t, service.DownloadPrivateCertChainViaUrl, nil, &apiKey, "test-c", "", output.JsonErrUnauthorized)
	// // `d` doesnt have a any correct apikey
	oneTest(t, service.DownloadPrivateCertChainViaUrl, nil, &apiKey, "test-e", `-----BEGIN RSA PRIVATE KEY-----

N/VC8wvORy4lKBkAeRy0oK+9o6R5mdzP3Fjpz9I4
-----END RSA PRIVATE KEY-----
-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----
-----BEGIN CERTIFICATE-----

g1/KbwHRr8PkIHTFtGbFNvZzjbrYb/FhHbSA
-----END CERTIFICATE-----
-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`, nil)
	oneTest(t, service.DownloadPrivateCertChainViaUrl, nil, &apiKey, "test-g", `-----BEGIN RSA PRIVATE KEY-----

j1f1P6e7Khe0uXD3N+r34piMQT0WX0po2rf16x0i
-----END RSA PRIVATE KEY-----
-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----
-----BEGIN CERTIFICATE-----

HZtRvh8TK5VI0J64ed//xMalyNSeh9gva5KB
-----END CERTIFICATE-----
-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`, nil)

	// Test: correct new apikey provided
	apiKey = "c-abc-new.k-123-new"
	oneTest(t, service.DownloadPrivateCertChainViaUrl, nil, &apiKey, "test-b", "", output.JsonErrUnauthorized)
	oneTest(t, service.DownloadPrivateCertChainViaUrl, nil, &apiKey, "test-e", `-----BEGIN RSA PRIVATE KEY-----

N/VC8wvORy4lKBkAeRy0oK+9o6R5mdzP3Fjpz9I4
-----END RSA PRIVATE KEY-----
-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----
-----BEGIN CERTIFICATE-----

g1/KbwHRr8PkIHTFtGbFNvZzjbrYb/FhHbSA
-----END CERTIFICATE-----
-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`, nil)

	// Test: correct apikey but api is disabled
	oneTest(t, service.DownloadKeyViaUrl, nil, &apiKey, "test-f", "", output.JsonErrUnauthorized)
}
