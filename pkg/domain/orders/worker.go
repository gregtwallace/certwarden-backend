...
func (service *Service) doOrderJob(workerId int, job orderJob) {
	...
	defer func(orderId int, service *Service) {
		...
		if err != nil {
			service.logger.Errorf("worker %d: error: %s", workerId, err)
		}
	}(job.orderId, service)
	...
	service.logger.Infof("worker %d: ordering order id %d (certificate name: %s, subject: %s)", workerId, orderDb.ID, orderDb.Certificate.Name, orderDb.Certificate.Subject)
...
fulfillLoop:
	for i := 1; i <= maxTries; i++ {
		...
		case "processing":
			...
			if i != maxTries {
				...
				case <-service.shutdownContext.Done():
					...
					service.logger.Errorf("worker %d: order job canceled due to shutdown", workerId)
					return
		...
		case "invalid": // break, irrecoverable
			service.logger.Infof("worker %d: order status invalid; acme error: %s", workerId, acmeOrder.Error)
			break fulfillLoop

		// Note: there is no 'expired' Status case. If the order expires it simply moves to 'invalid'.

		default:
			service.logger.Errorf("worker %d: error: order status unknown", workerId)
			return // done, failed
		}
	}
	...
	if service.isHttps &&
		...
		*service.serverCertificateName == orderDb.Certificate.Name &&
		...
		if err != nil {
			service.logger.Errorf("worker %d: failed to load lego's new https certificate (%s)", workerId, err)
		} else {
			service.logger.Debugf("worker %d: new lego https certificate loaded", workerId)
		}
	}

	// always info log order completed
	service.logger.Infof("worker %d: order id %d completed as %s (certificate name: %s, subject: %s)", workerId, orderDb.ID, acmeOrder.Status, orderDb.Certificate.Name, orderDb.Certificate.Subject)
}
